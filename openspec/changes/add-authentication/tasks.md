...
- [ ] 1.1 Instalar `react-router` e verificar que `npm install` conclui sem
      aviso de par incompatível com React 19 e que `package.json` e
      `package-lock.json` registram a versão instalada.
- [ ] 1.2 Criar os diretórios `src/auth/` e `src/pages/` e verificar que a
      aplicação continua subindo com `npm run dev` antes de qualquer mudança
      de comportamento.

## 2. Cliente HTTP

- [ ] 2.1 Criar `src/api/ApiError.js` com `status`, `code`, `message` e
...
- [ ] 2.2 Criar `src/api/client.js` montando a URL a partir de `API_URL`,
      ...
      erro de leitura.
- [ ] 2.3 Fazer o cliente converter em `ApiError` toda falha — erro do
...
- [ ] 2.4 Fazer o cliente anexar `Authorization: Bearer` nas chamadas
...
- [ ] 3.1 Criar `src/auth/tokenStore.js` guardando os tokens e o usuário
...
- [ ] 3.2 Envolver toda leitura e escrita do armazenamento em `try/catch`;
...
- [ ] 3.3 Expor no depósito a notificação de sessão perdida, para que o
      contexto se inscreva; verificar que limpar os tokens pelo depósito
      avisa o inscrito exatamente uma vez.

## 4. Renovação automática

- [ ] 4.1 Implementar no cliente a reação ao `401` em chamada autenticada:
...
- [ ] 4.2 Garantir que a renovação não é tentada para endpoints públicos nem
...
- [ ] 4.3 Implementar o disparo único guardando a promessa da renovação em
...
- [ ] 4.4 Fazer a renovação recusada encerrar a sessão localmente e reportar
      ...
      protegida em erro.
- [ ] 4.5 Verificar o caso do modo estrito de desenvolvimento: com o backend
...
- [ ] 5.1 Criar `src/api/auth.js` com entrada, cadastro, renovação e saída, e
...
- [ ] 5.2 Reescrever `src/api/health.js` sobre o cliente, preservando o
...
- [ ] 6.1 Criar `src/auth/AuthContext.jsx` com o provedor e os estados
...
- [ ] 6.2 Criar `src/auth/useAuth.js` como único acesso das telas à sessão;
      verificar por busca no projeto que nenhuma tela importa o depósito de
      tokens nem lê o `localStorage` diretamente.
- [ ] 6.3 Implementar a restauração de sessão na montagem: com tokens
...
- [ ] 6.4 Distinguir, na restauração, backend inacessível de sessão inválida;
      verificar com o backend desligado que a sessão guardada não é apagada e
      a falha é apresentada como problema de conexão.
- [ ] 6.5 Implementar entrar e cadastrar no contexto, com o cadastro seguido
      ...
      sessão.
- [ ] 6.6 Tratar o caso de registro aceito e entrada seguinte falha;
      ...
      falha no cadastro.
- [ ] 6.7 Implementar sair, revogando no backend e descartando estado e
...
- [ ] 6.8 Inscrever o contexto na notificação de sessão perdida do depósito;
...
- [ ] 7.1 Criar o mapa de rotas conforme a tabela da decisão 6 de
...
- [ ] 7.2 Reescrever `src/App.jsx` para montar o provedor de sessão e o
      ...
      galeria.
- [ ] 7.3 Criar a guarda de rota com os três estados — exibir, carregar,
      ...
      entrada.
- [ ] 7.4 Implementar o retorno ao destino pretendido pelo `state` do
      ...
      leva à tela inicial.
- [ ] 7.5 Redirecionar quem tem sessão ativa para fora de `/entrar` e
      `/cadastro`; verificar que abrir essas URLs autenticado leva à tela
      inicial.
- [ ] 7.6 Criar a tela de rota não encontrada com caminho de volta;
      verificar que uma URL inexistente a exibe, em vez de tela em branco.

## 8. Telas de entrada e cadastro

- [ ] 8.1 Criar `src/pages/LoginPage.jsx` com e-mail, senha, ação de entrar e
...
- [ ] 8.2 Criar `src/pages/RegisterPage.jsx` com nome, e-mail, senha, ação de
      criar conta e caminho para a entrada; verificar que um cadastro válido
      leva à área autenticada.
- [ ] 8.3 Implementar a validação no cliente espelhando as regras do backend
...
- [ ] 8.4 Exibir os erros do backend no campo correspondente, usando o mapa
...
- [ ] 8.5 Exibir como erro geral, anunciado a tecnologias assistivas, o que
...
- [ ] 8.6 Implementar o estado de envio em curso bloqueando reenvio;
...
- [ ] 8.7 Garantir a acessibilidade dos formulários — rótulo associado,
...
- [ ] 9.1 Criar `src/pages/HomePage.jsx` exibindo quem está autenticado;
      verificar que o nome e o e-mail vindos do backend aparecem após entrar.
- [ ] 9.2 Oferecer a ação de sair alcançável de qualquer tela protegida;
...
- [ ] 9.3 Mover a tela de status do backend para `/diagnostico` como rota
      pública e a galeria para `/galeria`; verificar sem sessão que as duas
      abrem normalmente.

## 10. Documentação e verificação final

- [ ] 10.1 Atualizar o `README.md` com o mapa de rotas, o fluxo de
...
- [ ] 10.2 Rodar `npm run lint` e `npm run build`; verificar que ambos passam
      sem aviso.
- [ ] 10.3 Exercitar o fluxo completo contra o backend local nos dois temas —
...
- [ ] 10.4 Exercitar o mesmo fluxo com o backend desligado; verificar que

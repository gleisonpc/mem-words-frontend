...
- [ ] 1.1 Criar `src/styles/tokens.css` com os tokens primitivos de cor (escala
...
- [ ] 1.2 Adicionar ao mesmo arquivo os tokens semânticos do tema claro
...
- [ ] 1.3 Adicionar o bloco `@media (prefers-color-scheme: dark)` reapontando
...
- [ ] 2.1 Medir, com a fórmula de contraste da WCAG 2.1, cada par listado em
...
- [ ] 3.1 Criar `Button` com as variantes primária, secundária, ghost e
...
- [ ] 3.2 Criar `Input` com rótulo, texto de ajuda e mensagem de erro, com o
      ...
      posicionada ao lado.
- [ ] 3.3 Criar `Card` com título opcional; verificar na galeria que o
      conteúdo aparece sobre superfície distinta do fundo da página nos dois
      temas.
- [ ] 3.4 Criar `Badge` com as variantes neutra, sucesso, atenção, erro e
      informação; verificar que cada variante identifica o estado pelo texto,
      e não apenas pela cor.
- [ ] 3.5 Criar `Alert` com as variantes de sucesso, atenção, erro e
...
- [ ] 3.6 Criar `Spinner` com rótulo textual acessível; verificar que o estado
      de carregamento é perceptível sem depender da animação.
- [ ] 3.7 Criar `ProgressBar` expondo valor atual, mínimo e máximo, limitando
      ...
      quebra.
- [ ] 3.8 Definir o estilo de foco visível compartilhado, aplicado a todo
...
- [ ] 5.3 Remover de `src/App.css` os estilos que migraram para os
...
- [ ] 6.1 Rodar `npm run lint` e `npm run build`; verificar que ambos passam
      sem aviso.
- [ ] 6.2 Exercitar a aplicação no navegador nos dois temas — tela de status e
...
- [ ] 6.3 Confirmar que nenhuma dependência foi adicionada; verificar que
